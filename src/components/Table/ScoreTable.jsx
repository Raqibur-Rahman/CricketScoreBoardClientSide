    ...
    <div className="container mx-auto p-4 pt-24">
      {matchData ? (
        <div>
          {matchData.map((match) => (
            <div key={match._id}>
              <h2 className="text-2xl font-bold mb-4">{`Match ID: ${match.matches[0].matchId}`}</h2>
              <p className="mb-2">{`Type: ${match.matches[0].type}`}</p>
              <p className="mb-4">{`Date: ${match.matches[0].date}`}</p>

              {/* Display batting and bowling scoreboards for each team */}
              {match.matches[0].teams.map((team, index) => (
